---
title: "Check 129 Proportion GWAS"
...
# Percentage of 129 that we used for each haplotype reconstruction.
pct_names = c(10, 15, 20, 30, 40, 50)
...
probs_files = file.path(hap_dir, str_c('atmko_hs_alleleprobs_cleaned_', pct_names ,'pct_129.rds'))
...
for(i in 1:length(probs_files)) {
  ...
  print("Heritability")
  est_herit(pheno = pheno[,'log_days',drop = F], kinship = allK)
  ...
  png(file.path(fig_dir, paste0('log_survival_', pct,'pct.png')), width = 1000, height = 800, res = 128)
  plot_snpasso(gwas$lod, gwas$snpinfo, main = paste('Pct 129', pct, 'Log Survival'))
  dev.off()
  ...
  png(file.path(fig_dir, paste0('rz_survival_', pct,'pct.png')), width = 1000, height = 800, res = 128)
  plot_snpasso(gwas$lod, gwas$snpinfo, main = paste('Pct 129', pct, 'RankZ Survival'))
  dev.off()
...
probs_files = file.path(hap_dir, str_c('atmko_hs_genoprobs_', pct_names ,'pct_129.rds'))
...
names(xo_list)   = pct_names
names(p129_list) = pct_names
