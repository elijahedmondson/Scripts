...
library(readxl)
all <- read_excel("Waldmann Project/MHL 212488 Waldmann.xlsx", 
                   sheet = "PLOT")
data <- all
...
# Plot
left.TDC1 <- ggparcoord(left, columns = 6:18, groupColumn = 3, scale="std", showPoints = TRUE, alphaLines = .8, splineFactor = F, title = "Left Tumor") +
...
########
my_mean = aggregate(data$'M1:M2', by=list(data$Group), mean, na.rm=TRUE) ; colnames(my_mean)=c("Group" , "mean")
my_CI = aggregate(data$'M1:M2' , by=list(data$Group) , FUN = function(x) t.test(x)$conf.int) ; colnames(my_CI)=c("Group" , "CI")
...
m1m2 <- ggplot(data) + 
  ...
  scale_y_continuous(name = "Tumor: M1:M2") +
  ...
  geom_jitter(aes(x = Group, y = data$'M1:M2', color = Side), width = 0.2, size = 3) +
  theme(axis.title.x=element_blank()) +
...
tiff("Plot111.tiff", units="in", width=20, height=10, res=600)
